...
export default function App() {
  ...
  const isSearchedResults = searchedSongs != null;

  useEffect(() => {
    fetchPoplularSongs();
  }, []);

  const fetchPoplularSongs = async () => {
    setIsLoading(true);
  ...
  return (
    <div className="flex flex-col min-h-screen bg-gray-900 text-white">
      <main className="flex-1 p-8 mb-20">
        ...
        <section>
          <h2 className="text-2xl font-semibold mb-5">{isSearchedResults ? "Searched Results" : "Popular Songs"}</h2>
          <SongList isLoading={isLoading} songs={isSearchedResults ? searchedSongs : popularSongs} onSongSelected={handleSongSelected}/>
          {isSearchedResults && <Pagination onPrev={hasPrev ? moveToPrev : null} onNext={hasNext ? moveToNext: null} />}
        </section>
      </main>
